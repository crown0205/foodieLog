...
export class PostService {
  ...
  async searchMyPostsByTitleAndAddress(
    ...
    const posts = await queryBuilder
      .andWhere(
        new Brackets((qb) => {
          qb.where('post.title like :query', { query: `%${query}%` }); // NOTE : like 연산자는 특정 문자열이 포함된 데이터를 찾을 때 사용합니다.
          qb.orWhere('post.address like :query', { query: `%${query}%` }); // NOTE : orWhere는 여러 조건 중 하나라도 참이면 참을 반환합니다.
        }),
      )
      .skip(offset)
      .take(perPage)
  ...
  async updatePost(
    ...
    const { title, description, color, date, score, imageUrls } = updatePostDto;
    post.title = title;
    ...
    const images = imageUrls.map((uri) => this.imageRepository.create(uri));
    post.images = images;
  ...
  async getPostsByMonth(year: number, month: number, user: User) {
    ...
    const groupPostsByDate = posts.reduce((acc, post) => {
      const { id, title, address, date, color } = post;

      if (!acc[date]) {
        acc[date] = [];
      }
      acc[date].push({ id, title, address, color });
